...
export default function Product() {
  ...
  let totalrange=selectpr.reviews && selectpr.reviews.reduce((c,e)=>c + e.rating , 0 )
  let avargerang=selectpr.reviews?.length && totalrange/selectpr.reviews.length
        ...
        <div className="pt-6">
          ...
          <nav aria-label="Breadcrumb">
            <ol className="mx-auto flex max-w-7xl items-center space-x-2 px-4 sm:px-6 lg:px-8">
              {selectpr.tags && selectpr.tags.map((breadcrumb) => (
          ...
          <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 lg:px-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
              ...
              <div className='sm:mx-5'>
                <h1 className="text-3xl font-bold tracking-tight text-gray-900">{selectpr.title}</h1>
    ...
    i < fullStars
      ...
      : i === fullStars && halfStar
        ...
        : <i key={i} className="far fa-star text-gray-300 text-lg"></i>        // ستاره خالی
                  ...
                  <span className="ml-3 text-sm font-medium text-indigo-600">{selectpr.reviews?.length} reviews</span>
                </div>

                {/* PRODUCT DETAILS */}
                <div className="mt-8 space-y-2">
                  <h3 className="text-xl font-semibold text-gray-900">Brand: {selectpr.brand}</h3>
                  <h2 className="text-lg font-bold text-gray-900">Dimensions:</h2>
                  <p className="text-gray-700">Depth: {selectpr.dimensions.depth}</p>
                  <p className="text-gray-700">Height: {selectpr.dimensions.height}</p>
                  <p className="text-gray-700">Width: {selectpr.dimensions.width}</p>
                </div>

                {/* PRICE */}
                <p className="my-5 text-3xl font-bold text-gray-900">Price: {selectpr.price}$</p>

                {/* BUTTON */}
                <button 
                 onClick={(e) =>{
                    e.stopPropagation(); 
                    carthandler(selectpr)}}
            ...
            <div className="mt-20 grid grid-cols-1 gap-10">
              <div>
                ...
                <p className="mt-4 text-gray-700">{selectpr.description}</p>
              </div>

              <div>
                <h3 className="text-xl font-bold mb-6 border-b border-gray-200 pb-2">Shipping Information</h3>
                <p className="mt-4 text-gray-700">{selectpr.shippingInformation}</p>
              </div>
            </div>
